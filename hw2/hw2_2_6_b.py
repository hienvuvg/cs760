...
X1 = np.arange(0,1.1,1.1/n_points)
X2 = np.arange(0,1.1,1.1/n_points)
...
for i in range(n_points):
    for j in range(n_points):
        X_test[j,:] = np.array([x1[i], x2[j]]).T
    pos[:,i] = cls.PredictTree(X_test) 
...
for g in np.unique(group):
    i = np.where(group != g)
    ax2.scatter(X[i, 0], X[i, 1], label=g, s=8, alpha=1)

ax2.contourf(X1, X2, pos, cmap='Spectral', alpha=0.2)
# plt.axis('equal')
plt.xlim([0,1])
plt.ylim([0,1])
plt.grid(color='gray', linestyle=':', linewidth=0.5)
plt.savefig(input_loc + '/figures/hw2_2_6_b_D2.pdf', format='pdf', bbox_inches='tight') # Must call before show()
plt.show()
