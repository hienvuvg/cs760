...
class DecisionTree():
    ...
    def FindBestSplit(self, dataset, n_features, depth):
        best_split = {}
        max_gain_ratio = -1
            ...
            for split_threshold in threshold_candidates:
                ...
                if len(left_group)>0 and len(right_group)>0:
                    ...
                    if gain_ratio > max_gain_ratio:
                        ...
                        best_split["depth"] = depth
                        # update max value
                        max_gain_ratio = gain_ratio

        # # print out best splits
        # if best_split["gain_ratio"] > 0:
        #     print('Split: X%d  >= %f  Depth: %d  GainRatio=%.3f' % (best_split["feature_index"], best_split["split_threshold"], best_split["depth"], best_split["gain_ratio"]))
    ...
    def MakeSubTree(self, data, depth=0): 
        features, values = data[:,:-1], data[:,-1]
        n_samples, n_features = np.shape(features)
        
        # only split when at lease two samples left, stop when it's empty
        if n_samples > 1:
            best_split = self.FindBestSplit(data, n_features, depth)
           
            if best_split["gain_ratio"] > 0: # stop if gain ratio is zero
                left_tree = self.MakeSubTree(best_split["left_group"], depth + 1)
                right_tree = self.MakeSubTree(best_split["right_group"], depth + 1)
    ...
    def PrintTree(self, DT=None, guide=""):
        if not DT:
            DT = self.root

        if DT.node_class is not None:
            ...
            print("%sElse: " % (guide), end="")
            self.PrintTree(DT.right_group, guide + "+-----")
...
data = pd.read_csv(input_loc +'/data/D2.txt', delimiter=' ',header = None)  # Also read the first row
